...
public sealed class InternalInvokeCommand
{
    ...
    /// <summary>Bare routeKey (e.g. "claude-opus-4.5"). Paired with <see cref="VendorKey"/> since routeKeys may repeat across channels.</summary>
    [Key(1)] public string ModelName { get; set; } = "";

    /// <summary>Idempotency key correlating with metering reservation on the Demux side.</summary>
    ...
    /// <summary>Channel key (NATS queue group). With <see cref="ModelName"/> (routeKey) it pins the exact route/rate row.</summary>
